...
│
├── Untitled.ipynb          # Main Jupyter notebook with complete implementation
├── glove.6B.100d.txt       # GloVe word embeddings (downloaded automatically)
├── glove.6B.zip            # GloVe archive (downloaded automatically)
└── README.md               # This file
```
...
Этот проект реализует модель бинарной классификации тональности для отзывов IMDb о фильмах с использованием методов глубокого обучения. Модель классифицирует отзывы как положительные или отрицательные, используя двунаправленную LSTM нейронную сеть с предобученными GloVe эмбеддингами слов.

## Описание проекта

Проект решает классическую задачу NLP - анализ тональности на датасете IMDb, который содержит 50,000 отзывов о фильмах, помеченных как положительные или отрицательные. Решение использует:
...
- Бинарная классификация тональности (положительная/отрицательная)
...
IMDb_sentiment_classification/
│
├── Untitled.ipynb          # Главный Jupyter ноутбук с полной реализацией
├── glove.6B.100d.txt       # GloVe эмбеддинги слов (загружаются автоматически)
├── glove.6B.zip            # Архив GloVe (загружается автоматически)
└── README.md               # Этот файл
```
